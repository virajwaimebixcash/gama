...
const Render = ({ config, index, entityName, tableData, setTableData, disabledform, userData, isTable }) => {

    const hiddenInputRef = useRef(null);
    const { t, i18n } = useTranslation();
    const [cleared, setCleared] = useState(false);
    const [url, setUrl] = useState(config.fetchdataapiurl)
    ...
    const renderInputField = (row, column, rowIndex) => {
        const name = column.dispatchername.split(".")[1]

        const userDataEntry = { ...userData?.[config.dispatchername]?.[rowIndex], rowId: Date.now() };

        // Set defaultValue from the found userDataEntry or set to an empty string if not found
        const defaultValue = userDataEntry ? userDataEntry[name] : '';

        return (
            <Render
                key={`${row.rowId}-${name}`}
                config={{
                    name: `${name}_${row.rowId}`,
                    component: column.component,
                    label: column.label,
                    placeholder: column.placeholder,
                    dispatchername: `${column.dispatchername}_${row.rowId}`,
                    validation: column.validation,
                    fetchdataapiurl: column.fetchdataapiurl, // For select/multiselect dropdowns
                    value: getValues(`${name}_${row.rowId}`),
                    defaultValue: defaultValue,
                    checkboxtext: column.checkboxtext,
                    allValues: getValues()
                }}
        ...
        switch (config?.component) {
            ...
            case "dynamicTable":
                return {
                    ...
                    rows: tableData[config.dispatchername],
                    renderInputField: renderInputField,
                    buttonAction: handleAddRow,
                    iconAction: handleRemoveRow,
                    ActionType: "Action",
                    defaultValue: userData?.[config.dispatchername] ?? '',
                    // tableName: config.dispatchername
                    tableName: config.dispatchername,
