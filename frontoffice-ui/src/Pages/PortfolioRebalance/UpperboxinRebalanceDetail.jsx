...
const UpperboxinRebalanceDetail = ({ config, gridData, selectedModelData, sortingState, setSortingState }) => {
  ...
  const handleCalculate = () => {
    const newValues = gridData.map((row, index) => {
      let calculatedValue = 0;
      if (selectedOption === "% of deviation amount") {
        calculatedValue = (row.deviationValue * parseFloat(percentageValue)) / 100;    
      } else if (selectedOption === "% of actual Holdings amount") {
        calculatedValue = (row.holdingValue * parseFloat(percentageValue)) / 100;
        console.log("Calculated Value:", calculatedValue);
      } else if (selectedOption === "% of target amount") {
        calculatedValue = (row.targetValue * parseFloat(percentageValue)) / 100;
      }
      return { ...row, approvedValue: Number(calculatedValue.toFixed(2)) };
    });
  ...
  return (
    <Box sx={{ flexGrow: 1 }}>
                          ...
                          <Grid className="minhegt50" size={{ xs: 4, sm: 4, md: 6, lg: 1, }} >
                            <div className='perinpbox'>
                              <TextField
                                ...
                                placeholder=''
                                size="small"
                                value={percentageValue} onChange={handlePercentageChange}
      ...
      <Grid container spacing={2}>
        <Grid size={12}>
          <Item >
            <DetailPageGridSection
              gridconfigdDetail={config?.gridColumnFields}
               gridData={gridData}
